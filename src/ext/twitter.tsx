...
export function setupTwitterObserver(config: ActionAdapter) {
  ...
  const observer = new MutationObserver((mutations) => {
    ...
    for (let i = 0; i < mutations.length; i++) {
      ...
      for (let j = 0; j < mutation.addedNodes.length; j++) {
        ...
        handleNewNode(node as Element, config).catch(noop);
      }
    }
  });
...
async function handleNewNode(node: Element, config: ActionAdapter) {
  ...
  rootElement.parentElement?.replaceChildren(
    createAction(actionUrl.toString(), action),
  );
}

function createAction(originalUrl: string, action: Action) {
  ...
  actionRoot.render(
    <ActionContainer action={action} websiteUrl={originalUrl} />,
  );
